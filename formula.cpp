...
namespace
{
    ...
    HelpTseitinData HelpTseitin(FormulaPtr f, std::shared_ptr<Minisat::Solver> solver)
    {
        ...
        std::transform(f->terms.begin(), f->terms.end(), insidePos.begin(), [solver](FormulaPtr i)
                       { return ApplyTseitin(i, solver); });
        std::transform(f->terms.begin(), f->terms.end(), insidePos.begin(), [solver](FormulaPtr i)
...
FormulaPtr ApplyTseitin(FormulaPtr f, std::shared_ptr<Minisat::Solver> solver)
{
    ...
    case Op::Op_And:
    {
        // (a∧b)を、fresh variable cをつかい(c ∧ (c<->a∧b))にする。後半は(c->a∧b ∧ a∧b->c)で、さらに(¬c∨(a∧b) ∧ (¬(a∧b)∨c))で、
        // 分配すると(¬c∨a ∧ ¬c∨b ∧ ¬a∨¬b∨c)になる。まとめると、(c ∧ ¬c∨a ∧ ¬c∨b ∧ ¬a∨¬b∨c)になる。これを複数個に一般化する。

        // 中身のabたちと、¬aや¬bを先に処理する。
        auto [freshPos, freshNeg, insidePos, insideNeg] = HelpTseitin(f, solver);
        ...
        newTerms.push_back(FormulaPtr{new Formula{Op::Op_Or, {}, lastPart}});
    ...
    {
        // (a∨b)を、fresh variable cをつかい(c ∧ (c<->a∨b))にする。後半は(c->a∨b ∧ a∨b->c)で、さらに(¬c∨a∨b ∧ (¬a∧¬b)∨c))で、
        // 分配すると(¬c∨a∨b ∧ ¬a∨c ∧ ¬b∨c)になる。これを複数個に一般化する。
    ...
    case Op::Op_Not:
    {
        ...
        case Op::Op_And:
        {
            ...
            std::transform(term->terms.begin(), term->terms.end(), newTerms.begin(), [](FormulaPtr i)
                           { return Formula::MakeNot(i); });
            return ApplyTseitin(FormulaPtr{new Formula{Op::Op_Or, {}, newTerms}}, solver);
        }
        break;
        case Op::Op_Atom:
        {
        ...
        case Op::Op_Or:
        {
            ...
            std::transform(term->terms.begin(), term->terms.end(), newTerms.begin(), [](FormulaPtr i)
                           { return Formula::MakeNot(i); });
            return ApplyTseitin(FormulaPtr{new Formula{Op::Op_And, {}, newTerms}}, solver);
        }
        break;
        default:
        {
